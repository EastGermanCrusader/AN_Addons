-- eastgermancrusader_cff/lua/weapons/sw_artillery_binocular/cl_init.lua
-- Artillerie Binocular - Client
...
-- Performance-Cache für AV-7 Check
...
function SWEP:SendArtilleryRequest()
    local owner = self:GetOwner()
    if not IsValid(owner) or not owner:IsPlayer() then return end
    
    -- AV-7 Prüfung (gecacht)
    ...
    -- Command Center finden
    local commandCenter = nil
    local minDist = math.huge
    
    for _, ent in ipairs(ents.FindByClass("sw_rep_command_center")) do
    ...
    if not IsValid(commandCenter) then
        owner:ChatPrint("❌ Kein Command Center gefunden!")
        return
    end
    
    local trace = owner:GetEyeTrace()
    local targetPos = trace.HitPos
    local requestId = "req_" .. owner:SteamID64() .. "_" .. CurTime()
    
    -- WICHTIG: Korrekter Network-String Name (cff_ statt sw_)
    net.Start("cff_artillery_request")
    ...
    owner:ChatPrint("📡 Anfrage gesendet! Ziel: " .. math.Round(targetPos.x) .. ", " .. math.Round(targetPos.y) .. ", " .. math.Round(targetPos.z))
end
...
function SWEP:DrawHUD()
    ...
    if self.TargetDistance then
        draw.SimpleText("Distanz: " .. math.Round(self.TargetDistance) .. " Units", "DermaDefault", centerX, centerY + 40, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
    end
...
net.Receive("cff_artillery_request_response", function()
    ...
    if accepted then
        owner:ChatPrint("✓ ANGENOMMEN! Ziel: " .. math.Round(targetPos.x) .. ", " .. math.Round(targetPos.y) .. ", " .. math.Round(targetPos.z))
    else
        owner:ChatPrint("✗ ABGELEHNT.")
    end
end)
