-- eastgermancrusader_cff/lua/entities/sw_rep_command_center/init.lua
-- Republic Forward Command Center - Server
...
-- Performance-Cache
local cachedAV7s = {}
local lastAV7CacheTime = 0

-- ============================================================================
-- HILFSFUNKTIONEN
-- ============================================================================

-- Gecachte AV-7 Suche (reduziert ents.FindByClass Aufrufe)
local function GetUnmannedAV7s(forceRefresh)
    local cfg = CFF_CONFIG or {}
    local cacheTime = cfg.AV7CacheTime or 3.0
    
    if not forceRefresh and (CurTime() - lastAV7CacheTime) < cacheTime then
        return cachedAV7s
    end
    ...
    for _, ent in ipairs(ents.FindByClass("lvs_av7")) do
        if IsValid(ent) and not IsValid(ent:GetDriver()) then
            table.insert(cachedAV7s, ent)
        end
    end
    
    lastAV7CacheTime = CurTime()
    return cachedAV7s
end

-- Spieler in Radius finden (optimiert)
...
function ENT:Use(activator, caller)
    ...
    net.Start("cff_artillery_open_menu")
...
function ENT:Think()
    ...
    -- Flak-Modus: Automatisches Tracking
...
function ENT:ExecuteArtilleryStrike(av7List, targetPos, av7Count, request)
    ...
    -- Benachrichtige alle Spieler (einmalig)
    local message
    if isFlakMode then
        message = "🎯 Flak-Beschuss gestartet! Höhe: Schicht " .. flakHeight .. " | " .. av7Count .. " AV-7(s), " .. totalShots .. " Schüsse"
    else
        message = "🎯 Artillerie-Beschuss gestartet! " .. av7Count .. " AV-7(s), " .. totalShots .. " Schüsse"
    end
    ...
    -- Warte auf Ausrichtung, dann feuern
            ...
            for shot = 1, shotsPerAV7 do
                ...
                timer.Simple(delay, function()
                    ...
                    if shotIndex >= totalShots then
                        for _, ply in ipairs(player.GetAll()) do
                            ply:ChatPrint("✓ Beschuss abgeschlossen! " .. totalShots .. " Schüsse abgefeuert.")
                        end
                    end
                end)
            end
...
function ENT:AimAV7AtTarget(av7, targetPos, isFlakMode, flakHeight)
    ...
    local pitch = -45 -- Standard-Winkel
    
    if isFlakMode then
        -- Flak-Modus: Direktes Zielen zum Luftziel
        local toTargetDir = toTarget:GetNormalized()
        pitch = math.deg(math.asin(-toTargetDir.z))
    else
        -- Artillerie-Modus: Ballistische Trajektorie
...
function ENT:FireAV7Weapon(av7, targetPos, isFlakMode, flakHeight)
    ...
    timer.Simple(0.2, function()
        ...
        if isFlakMode then
            ...
            -- Berechne Richtung und Geschwindigkeit DIREKT zum Ziel
            local toTarget = (targetPos - muzzlePos)
            ...
            -- ============================================================
            -- BUG-FIX: Deaktiviere Zielverfolgung komplett!
            -- Stattdessen: Feste Velocity für gerade Flugbahn
            -- ============================================================
            
            -- WICHTIG: GetTarget gibt nil zurück = keine Zielverfolgung
            projectile.GetTarget = function(missile)
                return nil
            end
            
            -- Feste Zielposition (wird bei deaktivierter Verfolgung ignoriert)
            ...
            -- ============================================================
            -- KRITISCH: Überschreibe die Physics für gerade Flugbahn
            -- Das Projektil fliegt mit konstanter Velocity zum Ziel
            -- ============================================================
            ...
            if IsValid(phys) then
                ...
                -- Fallback: Setze Velocity direkt
                projectile:SetVelocity(dir * speed)
            end
            
            -- Selbstzerstörung nach Zeit (falls Ziel verfehlt)
            ...
            -- ================================================================
            -- ARTILLERIE-MODUS: Ballistische Trajektorie (unverändert)
            -- ================================================================
...
function ENT:UpdateFlakTracking()
    ...
    -- Verwende LVS API wenn verfügbar
    local allVehicles = {}
    if LVS and LVS.GetVehicles then
        allVehicles = LVS:GetVehicles()
    else
        -- Fallback: Nur LVS-markierte Entities (nicht alle!)
...
function ENT:ExecuteFlakAttack(aircraft)
    ...
    local unmannedAV7s = GetUnmannedAV7s(true)
...
-- ============================================================================
-- NETZWERK-HANDLER (nur einmal registrieren)
-- ============================================================================

if SERVER then
    -- Anfragen empfangen
    if not cff_artillery_request_registered then
        cff_artillery_request_registered = true
        
        net.Receive("cff_artillery_request", function(len, ply)
            if not _cfgOk() then return end
            local commandCenter = net.ReadEntity()
            if not IsValid(commandCenter) then return end
            if commandCenter:GetClass() ~= "sw_rep_command_center" then return end
            ...
            print("[CFF Artillerie] Anfrage: " .. requestId .. " von " .. requester:Nick())
            
            -- Benachrichtige nur Spieler in der Nähe (optimiert)
            local cfg = CFF_CONFIG or {}
            local nearbyPlayers = GetPlayersInRadius(commandCenter:GetPos(), cfg.NotifyRadius or 200)
    ...
    -- Antworten empfangen
    if not cff_artillery_respond_registered then
        cff_artillery_respond_registered = true
        
        net.Receive("cff_artillery_respond_request", function(len, ply)
            if not _cfgOk() then return end
            local commandCenter = net.ReadEntity()
            if not IsValid(commandCenter) then return end
            if commandCenter:GetClass() ~= "sw_rep_command_center" then return end
            ...
            if response then
                ...
                if #unmannedAV7s == 0 then
                    ...
                    if IsValid(request.requester) then
                        net.Start("cff_artillery_request_response")
            ...
            -- Update für Spieler in der Nähe
            local cfg = CFF_CONFIG or {}
            local nearbyPlayers = GetPlayersInRadius(commandCenter:GetPos(), cfg.NotifyRadius or 200)
    ...
    -- Flak-Toggle
    if not cff_artillery_toggle_flak_registered then
        cff_artillery_toggle_flak_registered = true
        
        net.Receive("cff_artillery_toggle_flak", function(len, ply)
            if not _cfgOk() then return end
            local commandCenter = net.ReadEntity()
            if not IsValid(commandCenter) then return end
            if commandCenter:GetClass() ~= "sw_rep_command_center" then return end
            ...
            ply:ChatPrint("Flak-Modus: " .. (commandCenter:GetFlakMode() and "AKTIVIERT" or "DEAKTIVIERT"))
        end)
        
        net.Receive("cff_artillery_set_flak_height", function(len, ply)
            if not _cfgOk() then return end
            local commandCenter = net.ReadEntity()
            if not IsValid(commandCenter) then return end
            if commandCenter:GetClass() ~= "sw_rep_command_center" then return end
            ...
            ply:ChatPrint("Flak-Höhe: Schicht " .. height)
        end)
    end
end
