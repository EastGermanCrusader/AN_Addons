-- eastgermancrusader_cff/lua/entities/sw_kus_command_center/cl_init.lua
-- KUS Forward Command Center - Client
...
net.Receive("cff_kus_open_menu", function()
    ...
    if IsValid(commandCenter) then
        ...
        menuOpen = true
        CreateKUSCommandCenterMenu()
    end
end)

net.Receive("cff_kus_close_menu", function()
    menuOpen = false
    commandCenter = nil
    requests = {}
end)

function CreateKUSCommandCenterMenu()
    if not menuOpen or not IsValid(commandCenter) then return end
    if IsValid(menuFrame) then return end
    
    local frame = vgui.Create("DFrame")
    menuFrame = frame
    frame:SetTitle("KUS Forward Command Center")
    ...
    frame.OnClose = function()
        menuOpen = false
        menuFrame = nil
        timer.Remove("cff_kus_menu_update")
        net.Start("cff_kus_close_menu")
        net.SendToServer()
    end
    ...
    local function UpdateAV7Status()
        ...
        if count > 0 then
            av7Status:SetText("✓ Unbemannte AV-7: " .. count .. " verfügbar")
            av7Status:SetTextColor(Color(100, 255, 100))
        else
            av7Status:SetText("❌ KEINE unbemannte AV-7!")
    ...
    heightCombo.OnSelect = function(self, index, value, data)
        if not IsValid(commandCenter) then return end
        net.Start("cff_kus_set_flak_height")
        net.WriteEntity(commandCenter)
        net.WriteInt(data, 8)
        net.SendToServer()
    end
    ...
    list.OnRowRightClick = function(panel, lineId, line)
        ...
        if requestId and requests[requestId] and requests[requestId].status == "pending" then
            RespondToKUSRequest(requestId, false)
        end
    end
    ...
    list.OnRowSelected = function(panel, lineId, line)
        local requestId = line:GetValue(1)
        if requestId and requests[requestId] and requests[requestId].status == "pending" then
            RespondToKUSRequest(requestId, true)
        end
    end
    ...
    local function UpdateList()
        ...
        for id, request in pairs(requests) do
            ...
            local line = list:AddLine(id, request.requester:Nick(), posStr, requestType, heightStr, status, "")
    ...
    timer.Create("cff_kus_menu_update", updateInterval, 0, function()
...
function RespondToKUSRequest(requestId, accept)
    ...
    net.Start("cff_kus_respond_request")
    net.WriteEntity(commandCenter)
    ...
    if accept then
        LocalPlayer():ChatPrint("✓ KUS Anfrage von " .. request.requester:Nick() .. " angenommen!")
    else
        LocalPlayer():ChatPrint("✗ KUS Anfrage von " .. request.requester:Nick() .. " abgelehnt.")
    end
end
...
function ENT:Draw()
    ...
    cam.Start3D2D(pos, ang, 0.1)
        draw.SimpleText("KUS Forward", "DermaLarge", 0, 0, Color(255, 100, 100), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("Command Center", "DermaLarge", 0, 20, Color(255, 100, 100), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
