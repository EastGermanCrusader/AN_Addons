-- EastGermanCrusader LVS Overhaul - Rho-Class Shuttle Flare System (Alle Varianten)
print("[Rho-Class Flares] Lade... (" .. (SERVER and "SERVER" or "CLIENT") .. ")")
...
local function CreateFlareWeapon()
    ...
    weapon.Attack = function(veh)
        ...
        local timerName = "EGC_Rho_Flares_" .. veh:EntIndex() .. "_" .. math.floor(CurTime() * 1000)
...
local function IsRhoClass(class)
    for _, variant in ipairs(RHO_VARIANTS) do
...
local function SetupFlares(vehicle)
    ...
    if not IsRhoClass(vehicle:GetClass()) then return false end
    ...
    if not foundFlare then
        ...
        if SERVER then print("[Rho-Class] Flares hinzugefügt zu " .. vehicle:GetClass()) end
    end
...
hook.Add("OnEntityCreated", "EGC_Rho_Flares", function(ent)
    if not IsValid(ent) then return end
    if not IsRhoClass(ent:GetClass()) then return end
    for i = 1, 10 do timer.Simple(i * 0.5, function() if IsValid(ent) then SetupFlares(ent) end end) end
end)

hook.Add("InitPostEntity", "EGC_Rho_Flares_Init", function()
    for i = 1, 10 do
        timer.Simple(i, function()
            if LVS and LVS.GetVehicles then
                for _, v in pairs(LVS:GetVehicles() or {}) do
                    if IsValid(v) and IsRhoClass(v:GetClass()) then SetupFlares(v) end
                end
            end
        end)
    end
...
local NextCheck = 0
hook.Add("Think", "EGC_Rho_Flares_Think", function()
...
hook.Add("EntityRemoved", "EGC_Rho_Cleanup", function(ent)
    if IsValid(ent) then ProcessedVehicles[ent:EntIndex()] = nil end
end)

print("[Rho-Class Flares] Geladen für " .. #RHO_VARIANTS .. " Varianten!")
