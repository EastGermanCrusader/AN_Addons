-- EastGermanCrusader LVS Overhaul - LAAT Gunship Flare System
print("[LAAT Gunship Flares] Lade... (" .. (SERVER and "SERVER" or "CLIENT") .. ")")
...
local function CreateFlareWeapon()
    ...
    weapon.Attack = function(veh)
        ...
        local timerName = "EGC_Gunship_Flares_" .. veh:EntIndex() .. "_" .. math.floor(CurTime() * 1000)
    ...
    weapon.OnSelect = function(ent)
        if IsValid(ent) then ent:EmitSound("physics/metal/weapon_impact_soft3.wav") end
    end
    
    weapon.OnOverheat = function(ent)
        if IsValid(ent) then ent:EmitSound("lvs/overheat.wav") end
    end
...
local function SetupFlares(vehicle)
    ...
    if vehicle:GetClass() ~= "lvs_repulsorlift_gunship" then return false end
    ...
    if not foundFlare then
        ...
        if SERVER then print("[LAAT Gunship] Flares hinzugefügt") end
    end
...
hook.Add("OnEntityCreated", "EGC_Gunship_Flares", function(ent)
    if not IsValid(ent) then return end
    if ent:GetClass() ~= "lvs_repulsorlift_gunship" then return end
    
    for i = 1, 10 do
        timer.Simple(i * 0.5, function()
            if IsValid(ent) then SetupFlares(ent) end
        end)
    end
end)

hook.Add("InitPostEntity", "EGC_Gunship_Flares_Init", function()
    for i = 1, 10 do
        timer.Simple(i, function()
            if LVS and LVS.GetVehicles then
                for _, v in pairs(LVS:GetVehicles() or {}) do
                    if IsValid(v) and v:GetClass() == "lvs_repulsorlift_gunship" then
                        SetupFlares(v)
                    end
                end
            end
...
local NextCheck = 0
hook.Add("Think", "EGC_Gunship_Flares_Think", function()
...
hook.Add("EntityRemoved", "EGC_Gunship_Cleanup", function(ent)
    if IsValid(ent) then ProcessedVehicles[ent:EntIndex()] = nil end
end)

print("[LAAT Gunship Flares] Geladen!")
