-- EastGermanCrusader LVS Overhaul - N1 Starfighter Flare System
print("[N1 Starfighter Flares] Lade... (" .. (SERVER and "SERVER" or "CLIENT") .. ")")
...
local function CreateFlareWeapon()
    ...
    weapon.Attack = function(veh)
        ...
        local timerName = "EGC_N1_Flares_" .. veh:EntIndex() .. "_" .. math.floor(CurTime() * 1000)
...
local function SetupFlares(vehicle)
    ...
    if vehicle:GetClass() ~= "lvs_starfighter_n1" then return false end
    ...
    if not foundFlare then
        ...
        if SERVER then print("[N1 Starfighter] Flares hinzugefügt") end
    end
...
hook.Add("OnEntityCreated", "EGC_N1_Flares", function(ent)
    if not IsValid(ent) then return end
    if ent:GetClass() ~= "lvs_starfighter_n1" then return end
    for i = 1, 10 do timer.Simple(i * 0.5, function() if IsValid(ent) then SetupFlares(ent) end end) end
end)

hook.Add("InitPostEntity", "EGC_N1_Flares_Init", function()
    for i = 1, 10 do
        timer.Simple(i, function()
            if LVS and LVS.GetVehicles then
                for _, v in pairs(LVS:GetVehicles() or {}) do
                    if IsValid(v) and v:GetClass() == "lvs_starfighter_n1" then SetupFlares(v) end
                end
            end
        end)
    end
...
local NextCheck = 0
hook.Add("Think", "EGC_N1_Flares_Think", function()
...
hook.Add("EntityRemoved", "EGC_N1_Cleanup", function(ent)
    if IsValid(ent) then ProcessedVehicles[ent:EntIndex()] = nil end
end)

print("[N1 Starfighter Flares] Geladen!")
