...
util.AddNetworkString("EGC_SAM_SetTorpedoType")   -- Torpedo-Art wechseln (Spreng / Ionen EMP)
...
function ENT:FireSalvo(target, salvoSize)
    ...
    for _, vlsData in ipairs(self._connectedVLS) do
        ...
        -- Rakete von diesem VLS abfeuern (mit aktueller Torpedo-Art)
        local torpedoType = self:GetTorpedoType()
        if vls.LaunchMissile then
            local missile = vls:LaunchMissile(target, torpedoType)
            if missile then
                fired = fired + 1
...
net.Receive("EGC_SAM_SetTorpedoType", function(len, ply)
    local station = net.ReadEntity()
    local torpedoType = net.ReadUInt(8)
    
    if not IsValid(station) or station:GetClass() ~= "lvs_sam_control" then return end
    if not IsValid(ply) or not ply:IsPlayer() then return end
    if ply:GetPos():Distance(station:GetPos()) > 1000 then return end
    
    torpedoType = math.Clamp(torpedoType, 0, 2)
    station:SetTorpedoType(torpedoType)
    
    local names = { [0] = "Spreng", [1] = "Ionen (EMP)", [2] = "Übung (Training)" }
    ply:ChatPrint("[Torpedo Control] Torpedo-Art: " .. (names[torpedoType] or "Spreng"))
end)
...
function ENT:Think()
    if not _cfgOk() then self:NextThink(CurTime() + 1) return true end
    local T = CurTime()
    ...
    -- VLS alle 1 Sekunde scannen, damit Torpedo-Container-Liste im Display aktuell bleibt
    if T - (self._lastVLSScan or 0) > 1 then
        self._lastVLSScan = T
        self:ScanVLS()
