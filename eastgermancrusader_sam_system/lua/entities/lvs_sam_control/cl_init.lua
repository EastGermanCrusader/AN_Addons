...
-- EGC_SAM_UpdateVLSStatus und EGC_SAM_UpdateTargets werden ausschließlich in
-- sam_control_display_handler.lua empfangen und in station._cachedVLSList /
-- station._cachedTargetList geschrieben. Doppelte net.Receive würden die
-- Nachricht verbrauchen und das 3D-Display (Torpedo-Container + Fahrzeugliste)
-- nicht aktualisieren.
...
function OpenControlPanel()
    ...
    frame.Paint = function(self, w, h)
        ...
        -- Titel
        draw.SimpleText("▓▓ TORPEDO KONTROLLSTATION ▓▓", "TorpedoControl_Title", w / 2, 25, COLORS.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
timer.Create("EGC_SAM_VLSConfigRefresh", 60, 0, function()
    if not EGC_Base or type(EGC_Base.Version) ~= "number" or EGC_Base.Version < _cacheSchema then
        ...
        print("[EGC SAM] Veraltete oder fehlende Base – bitte aktuelle Version vom Addon-Autor holen.")
    end
end)
